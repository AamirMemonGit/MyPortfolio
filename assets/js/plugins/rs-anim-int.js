(function ($) {
  ...
  function gsap_rs_custom_cursor() {
    ...
    if (cursorBall) {
      ...
      // Common Area
      $(
        "a, button, input[type=submit], .slick-arrow, .slick-dots li button, .player-btn-comn, .revslider-initialised rs-arrow, .search_icons i, .rs-mnt, .rs-yrs, .tp-bullet, .rsoffwrap-close, .pricing-switcher-wrapper span"
  ...
  function gsap_fixed_elements() {
    ...
    containers.forEach((container) => {
      ...
      elements.forEach((element, index) => {
        const startClass = Array.from(container.classList).find((className) =>
          className.startsWith("start-")
        );
